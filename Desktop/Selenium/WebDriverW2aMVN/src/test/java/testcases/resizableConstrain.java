...
public class resizableConstrain {
	

	public static void resizableCon(String browser, String user, String password){
		...
		WebElement constraint = driver.findElement(By.xpath("//*[@id='container']"));
		int consWidth = constraint.getSize().getWidth();
		int consHeight = constraint.getSize().getHeight();
		System.out.println(resWidth + " " + resHeight);
		System.out.println(consWidth + " " + consHeight);
		letsWait(2);
		
		WebElement resize = driver.findElement(By.xpath("/html/body/div/div/div[3]"));
		
		
		
		Actions action = new Actions(driver);
	 
		action.clickAndHold(resize).moveByOffset(consWidth-resWidth-1, consHeight-resHeight-1).release().build().perform();
		letsWait(2);
		...
		if(resWidth < resWidthAfter && resHeight < resHeightAfter && resWidthAfter < consWidth && resHeightAfter < consHeight){
